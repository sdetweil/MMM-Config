...
for (let m of defines.config.modules) {
  ...
  if (value[m.module] !== undefined) {
    ...
    value[m.module] = JSON.parse(
      JSON.stringify(value[m.module], tohandler).replace(/"\.*/g, '"'),
      fromhandler
    );
    ...
    if (x) {
      ...
      if (debug)
        console.log("DO NOT have module info in config.js =" + m.module);
      position_hash[m.module] = {
        name: m.module,
...
Object.keys(pairVariables).forEach((m) => {
  ...
  for (let x of Object.keys(
    module_properties === "config" ? defines.config[varname] : module_properties
  )) {
    let existing_value;
    existing_value =
...
empty_arrays.forEach((key) => {
  ...
  if (module_data) {
    if (debug)
      console.log(
        "found value data for empty array definitions=" +
          key +
          " value=" +
          JSON.stringify(module_data, tohandler, 2) +
          " type=" +
          typeof module_data
      );
      ...
      if (module_data.length) {
        if (debug)
          console.log(
            "found value data for empty array definitions=" +
              key +
              " item is array, and has elements"
          );
        // get the 1st one
        ...
        if (debug)
          console.log(
            "found value data for empty array definitions=" +
              key +
              " item is array, and has elements, type=" +
...
form[0].items[1].items.forEach((m) => {
  if (debug)
    console.log(
      "module =" + m.title + " value data =" + value[m.title].disabled
    );
  m.htmlClass =
    value[m.title].disabled !== undefined && value[m.title].disabled === true
      ? "module_disabled"
      : "module_enabled"; // +' module_name_class'
});

value = JSON.parse(
  JSON.stringify(value, tohandler).replace(/"\.*/g, '"'),
  fromhandler
);
...
function processModule(schema, form, value, defines, module_definition) {
  ...
  mform.items.push({
    ...
    onChange:
      "(evt,node)=>{var selection=$(evt.target).prop('checked');var parent =$(evt.target).closest('fieldset');parent.find('legend').first().css('color',selection?'" +
