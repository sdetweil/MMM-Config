...
if(debug)
	console.log("defines="+JSON.stringify(defines," ",2))
...
if(add_helper_vars){
	...
	let x=t.shift()
	if(debug)
		console.log("end of defines list='"+x+"'")
	for(const k of Object.keys(other_vars)){
		if(debug)
			console.log("used variable not defined="+k+" default value ="+other_vars[k])
...
function readeFile(fn){
  ...
  if (fs.existsSync(fn)) {
  	// read it in and split it by new lines
		fs.readFileSync(fn, 'utf-8').split(/\r?\n/).forEach((line) =>{
	...
	if(lines.length <10){
		...
		for(let line of lines){
			//while(line.indexOf(';')>=0){
				if(debug) console.log("replacing type="+typeof line+"+ length="+line.length)
...
function process_main(lines, name ){
	...
	for(let line of  lines){
		...
		if (!started && line.includes(start_string)){// && !line.trim().startsWith('//')){
...
function process_helper(defines){
	...
	let located={}
	if(debug)
		console.log("testing  node_helper")
	lines.forEach((file_line)=>{
			if((index=file_line.indexOf(variable_use))>-1){
				let rest=file_line.substring(index+variable_use.length).replace(/[)?.,+&}[\]]/g,"                     ")
